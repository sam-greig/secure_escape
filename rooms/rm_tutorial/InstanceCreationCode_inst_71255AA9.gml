...
function interact() {
	...
	if (!subtitles_triggered) {
		...
		with (obj_user_interface) {
			...
			text_index = 0;                    
			typing_speed = 40; 
		    dialog = ["This is an interactable keypad. Move around using your direction keys and pressing your interact key to select a key.",
					  "To find out the correct passcode for the keypad it will be hidden in the room or a puzzle may need to be solved to discover it.",
					  "But since this is the tutorial the correct passcode is 1111. :)"];
		    show_subtitles = true;
		}
		subtitles_triggered = true;
	}
	if(!obj_user_interface.show_subtitles and !subtitles_finished){
		keyboard_clear(global.controls[4].value);
		subtitles_finished = true;
	}
	if(subtitles_finished){
